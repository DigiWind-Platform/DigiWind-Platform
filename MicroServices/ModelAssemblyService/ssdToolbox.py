...
E = builder.ElementMaker(nsmap=ns, namespace=ns["ssd"])

elementMakerSSC = builder.ElementMaker(nsmap=ns, namespace=ns["ssc"])

elementMakerDGW = builder.ElementMaker(nsmap=ns, namespace=ns["dgw"])
...
def createComponent(modelName, modelData, signalSet, server=""):
    ...
    if server:
        modelPath = getFTPLocation(server, modelData.description["identifier"][4:])
    else:
...
def createBaseSSD():
    ...
    ssd = SystemStructureDescription(
        system=system, name="SystemStructureDescription", version="1.0"
    )
    return ssd
